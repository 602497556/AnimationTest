...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state){
        ...
        if(position > -1){
            if(position == 0){
                //等于0肯定要有title
                outRect.set(0,mTitleHeight,0,0); //设置四个方向上需要为 ItemView设置padding的值
            } else {
                if(mDatas.get(position).getTag() != null && mDatas.get(position).getTag()
                        != mDatas.get(position-1).getTag()){
...
//                            parent.getRight() - parent.getPaddingRight(), parent.getPaddingTop() +
