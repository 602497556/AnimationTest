package com.example.animationtest.LauncherView;
...
public class LauncherView extends RelativeLayout {
    ...
    private int dp80 = SizeUtils.dp2px(getContext(),80);

    ImageView red, yellow, blue, purple;
    ...
    private void setAnimation(ImageView target, ViewPath path1, ViewPath path2) {
        //左右平移
        ObjectAnimator anim1 = ObjectAnimator.ofObject(new ViewObj(target),
                "fabLoc", new ViewPathEvaluator(),path1.getPoints().toArray());
        anim1.setInterpolator(new AccelerateDecelerateInterpolator());
        anim1.setDuration(800);
        //贝塞尔曲线
        ObjectAnimator anim2 = ObjectAnimator.ofObject(new ViewObj(target),
                "fabLoc", new ViewPathEvaluator(),path2.getPoints().toArray());
    ...
    private void showLogo(){
        ...
        final View slogan = view.findViewById(R.id.iv_slogo);
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                ObjectAnimator sAlpha = ObjectAnimator.ofFloat(slogan,View.ALPHA,0f,1f);
                sAlpha.setDuration(200);
                sAlpha.start();
            }
        },400);
    }
    ...
    public class ViewObj {
        ...
        public ViewObj(ImageView iv){
            this.iv = iv;
        }

        public void setFabLoc(ViewPoint newLoc){
            iv.setTranslationX(newLoc.x);
            iv.setTranslationY(newLoc.y);
        }
